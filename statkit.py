...
#       Toolkit.showProportion(lenDict, picName, picTitle)
    ...
    @staticmethod
    def statAndShowOperator(stats, fopr="", picName="", 
            picTitle="The Detailed Operator Distribution", 
            xlabel="Frequency of Detailed Operator", ylabel="Detailed Operator"):
        oprDict = {}
        for stat in stats:
            opr = stat['instr']
            if oprDict.has_key(opr):
                oprDict[opr] += 1
            else:
                oprDict[opr] = 1
        if len(fopr) > 0:
            Toolkit.writeJsonFile(fopr, oprDict)
#       Toolkit.showHistogram(oprDict, picName, picTitle, xlabel, ylabel)
        return oprDict
